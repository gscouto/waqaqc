...
def process_vorbin_pixel(idx_i, pix, vorbin_map, r_bin_id, bin_id,
    ...
    try:
        ...
        if not bin_mask.any():
            raise ValueError(f"Empty bin_mask for bin_val={bin_val}")

        factor = bin_pixel_counts.get(bin_val, 1)  # avoid division by 0

        if factor == 0:
            raise ZeroDivisionError(f"factor=0 for bin_val={bin_val}")
...
def cube_creator(ob, args):
    ...
    files = file_dir + [x for x in os.listdir(file_dir) if ('LWVE' in x)]
    heaviest_file = max(files, key=lambda f: os.path.getsize(files))
    cube = fits.open(file_dir + heaviest_file)
    ...
    if 'PATCH_ALLSPEC' in cube:
        ...
        args_cube = [(pix_mapt[i], apsid_map, aps_id, rss_data, rss_err)
                     for i in range(len(pix_mapt))]
    ...
    args_cube = [(cnt, pix_mapt[cnt], vorbin_map, bin_id, r_bin_id, cube['GALAXY_TABLE'].data, aps_maps_names)
    ...
    cpix = [cube['PATCH_TABLE'].data['X_0'][0] + (min(cube['PATCH_TABLE'].data['X'], key=abs) / 3600),
            cube['PATCH_TABLE'].data['Y_0'][0] + (min(cube['PATCH_TABLE'].data['Y'], key=abs) / 3600)]
