...
for gal in gals_id:
    ...
    res_dir = np.sort(os.listdir(result_dir+gal+'/'))[-1]

    gal_dir = gal+'/'+res_dir
    
    os.makedirs(gal_dir, exist_ok=True)

    c = fits.open('/work1/gcouto/weave/datacubes/IFU_APERTIF_TEST/'+gal+'.fits')
    rss_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin_RSS.fits')
    
    contm_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.cont_model.fits')
    contr_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.cont_res.fits')
    elinm_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.eline_model.fits')
    elinr_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.eline_res.fits')
    
    elint_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.eline_table.fits')
    stelt_file = fits.open(result_dir+gal+'/'+res_dir+'/'+gal+'_vorbin.stellar_table.fits')
    
    params_stel =  open(result_dir+gal+'/'+res_dir+'/parameters_stellar','r')
    ...
    stel_template = fits.open(template_dir+lines[1].split()[1])

    wave = np.exp(c[1].data['LOGLAM'][0])
    n_wave = np.arange(min(wave),max(wave),0.1)
    ...
    for i in np.arange(np.unique(xx).shape[0]):
        for j in np.arange(np.unique(yy).shape[0]):
            ...
            if vorbin_map[j,i] >= 0:
                ...
                print('Rearranging results into datacube formats: '+str(round(100.*i/np.unique(xx).shape[0],2))+'%',end='\r')
    ...
    base_coeff_maps = np.reshape(base_coeff_maps,(len(base_coeff_maps),base_coeff_maps[0].shape[0],base_coeff_maps[0].shape[1]))
    ...
    for k in r_bin_id:
        ...
        for i in np.arange(len(base_coeff_maps)):
            base_coeff_maps[i][nvorbin_map == k] = stelt_file[1].data['base_coeff'][stelt_file[1].data['fiber'] == k][0][i]
    ...
    cube_head['CRPIX1'] = np.where(np.unique(c[2].data['X']) == np.min(np.unique(c[2].data['X'])[np.unique(c[2].data['X'])>0]))[0][0]+1 # X from APS with lower absolute value
    cube_head['CRPIX2'] = np.where(np.unique(c[2].data['Y']) == np.min(np.unique(c[2].data['Y'])[np.unique(c[2].data['Y'])>0]))[0][0]+1 # Y from APS with lower absolute value
    cube_head['CRVAL1'] = np.unique(c[2].data['X_0'])[0] + (np.min(np.unique(c[2].data['X'])[np.unique(c[2].data['X'])>0])/3600.) # X from central pixel plus position X_0
    cube_head['CRVAL2'] = np.unique(c[2].data['Y_0'])[0] + (np.min(np.unique(c[2].data['Y'])[np.unique(c[2].data['Y'])>0])/3600.) # Y from central pixel plus position Y_0
    ...
    map_head['DISPAXIS'] = 1
    map_head['CRPIX1'] = np.where(np.unique(c[2].data['X']) == np.min(np.unique(c[2].data['X'])[np.unique(c[2].data['X'])>0]))[0][0]+1 # X from APS with lower absolute value
    map_head['CRPIX2'] = np.where(np.unique(c[2].data['Y']) == np.min(np.unique(c[2].data['Y'])[np.unique(c[2].data['Y'])>0]))[0][0]+1 # Y from APS with lower absolute value
    map_head['CRVAL1'] = np.unique(c[2].data['X_0'])[0] + (np.min(np.unique(c[2].data['X'])[np.unique(c[2].data['X'])>0])/3600.) # X from central pixel plus position X_0
    map_head['CRVAL2'] = np.unique(c[2].data['Y_0'])[0] + (np.min(np.unique(c[2].data['Y'])[np.unique(c[2].data['Y'])>0])/3600.) # Y from central pixel plus position Y_0
    ...
    hdu_base_coeff_maps = fits.HDUList([fits.PrimaryHDU(),fits.BinTableHDU(base_coeff_t,name=lines[1].split()[1][:-5])])
    ...
    n_elinm.writeto(gal_dir+'/'+gal+'_eline_model.fits',overwrite=True)
    n_contm.writeto(gal_dir+'/'+gal+'_cont_model.fits',overwrite=True)
    n_elinr.writeto(gal_dir+'/'+gal+'_eline_res.fits',overwrite=True)
    n_contr.writeto(gal_dir+'/'+gal+'_cont_res.fits',overwrite=True)

    n_tab_eline.writeto(gal_dir+'/'+gal+'_eline_table.fits',overwrite=True)
    n_tab_stell.writeto(gal_dir+'/'+gal+'_stellar_table.fits',overwrite=True)
    
    hdu_elint_maps.writeto(gal_dir+'/'+gal+'_eline_maps.fits',overwrite=True)
    hdu_stelt_maps.writeto(gal_dir+'/'+gal+'_stellar_maps.fits',overwrite=True)
    hdu_base_coeff_maps.writeto(gal_dir+'/'+gal+'_base_coeff_maps.fits',overwrite=True)
